...
public class BatchController {
	...
	public void BatchConstruct() {
		...
		com.batch.app.GlobalValueables.consumer = new KafkaConsumer<>(properties, new StringDeserializer(),
	...
	public void start() {
		// 停止ステータス以外だったら終了
		if (batchService.checkStatus0() == false) {
			return;
		}
		// ステータスを「1」に変更
		...
		try {
			...
			STANDBY: while (true) {
				...
				if (batchService.checkStatus2() == false) {
					...
					if (batchService.checkStatus0() == true) {
						break STANDBY;
					}
					continue STANDBY;
				}
				...
				EXECUTING: while (true) {
					try {
						// コンシューム
						com.batch.app.GlobalValueables.consumer
								.subscribe(Arrays.asList(com.batch.app.GlobalValueables.topicname));
						// 1秒毎に確認し、コンティニューする
						ConsumerRecords<String, String> records = com.batch.app.GlobalValueables.consumer
						...
						for (ConsumerRecord<String, String> record : records) {
							...
							message = com.batch.app.GlobalValueables.mapper.readValue(record.value(),
									FurikomiExecMessage.class);
